...
#define GOT_READ_MASK( tran_id )     (1 << (tran_id))
#define GOT_WRITE_MASK( tran_id )    (1 << (tran_id))
...
void tm_lock_block_readers( tm_lock_t* _lock )
{
	...
	if (_lock->rlock & (1 << WRITE_BIT))
	   return;

	do {	tmp  = _lock->rlock;	}
	while( cas( &_lock->rlock, tmp, tmp | (1 << WRITE_BIT) ) != tmp );
}
...
uint_t is_conflict_rw( uint_t c_id, uint_t tran_id, uint_t trail )
{
	...
	clear_mask( tmp, (1 << WRITE_BIT) | (1 << c_id) );
	if( !tmp )								
	   return 0;
    ...
    {
        if( is_set( tmp, i ) && deadlock_table[ i*16 ] && ( deadlock_trail = is_conflict_rw( i, tran_id, trail | (1 << i) ) ) )
...
uint_t is_deadlock_rw( tm_lock_t* _lock, uint_t tran_id )
{
	if( !deadlock_table[tran_id*16] )
	{
		...
		return is_conflict_rw( tran_id, tran_id, 1 << tran_id);
	}
	else return 0;
...
void tm_lock_waitfor_readers( tm_lock_t* _lock, uint_t tran_id )
{
	...
	while( _lock->rlock & (~((1 << WRITE_BIT) | (1 << tran_id))) )
	{
		if( is_deadlock_rw( _lock, tran_id ) )
		{
			atomic_clear_mask( ((1 << WRITE_BIT) | (1 << tran_id)), &(_lock->rlock) );
			ABORT_TRANSACTION(1);
		}
...
uint_t is_conflict_w( uint_t c_id, uint_t tran_id, uint_t trail )
{
	...
	if( deadlock_table[ i*16 ] && ( deadlock_trail = is_conflict_w( i, tran_id, trail | (1 << i) ) ) )
		return deadlock_trail;
...
uint_t is_deadlock_w( tm_lock_t* _lock, uint_t tran_id )
{
	...
	{
		atomic_set_mask( _lock, &deadlock_table[tran_id*16] );
		return is_conflict_w( tran_id, tran_id, 1 << tran_id);
	}
	else return 0;
...
void tm_lock_invalidate_readers( tm_lock_t* _lock, uint_t tran_id )
{
	...
	readers = _lock->rlock;
	clear_mask( readers, (1 << WRITE_BIT) | (1 << tran_id) );
	...
	if( readers )
	{
		for( i = 0; i < MAX_NO_THREADS; i++ )
			if( is_set( readers, i ) && !aborted_table[ i*16 ] )
				atomic_set_mask( 1, (&aborted_table[ i*16 ]) );
	}
}
...
void tm_lock_wrunlock( tm_lock_t* _lock, uint_t tran_id )
{
	...
	{
		if( _lock->rlock & (~((1 << WRITE_BIT) | (1 << tran_id))) )				// other readers exist
			atomic_clear_mask( ((1 << WRITE_BIT) | (1 << tran_id)), &_lock->rlock );
		else
			clear_mask( _lock->rlock, ((1 << WRITE_BIT) | (1 << tran_id)) );
	}
