...
int inter[NUM_PARTICLES][MAXNEIGHBOURS];
...
int Update( void* arg, int id )
{
  ...
  for ( i = id; i < numMoles; i += NTHREADS) 
  {
   ...
   /* if (i == 5)
    {
       printf ("\n%lf %lf %lf %lf %lf %lf\n", vh[ IND(0,i) ], vh[ IND(1,i) ], vh[ IND(2,i) ], force_x, force_y, force_z);
    }
    */
    //debug end
  ...
  for ( tstep=0; tstep< NUMBER_TIMESTEPS; tstep++) 
  {
    ...
    if ( tstep % neighUpdate == 0) 
    {
      ...
      if (id == 0) {
       ...
       if (id == 0) {
            for (i = 0; i < num_interactions[5]; ++i)
           {
              printf ("inter[5][%d]: %d\n", i, inter[5][i]);
           }
      }
    ...
    if (id == 0)
    {
      printf("\n\n velocity: %lf, %lf, %lf, forces: %lf, %lf, %lf, x: %lf, %lf, %lf\n", 
      vh[IND(0,240)], vh[IND(1,240)], vh[IND(2,240)], f[IND(0,240)], f[IND(1,240)], f[IND(2,240)], 
      x[IND(0,240)], x[IND(1,240)], x[IND(2,240)]);
    }
   */
