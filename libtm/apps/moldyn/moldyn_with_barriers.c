...
#define NTHREADS 4
#define NDIM 3
//enable this when we want to bypass barriers between ComputeForces and Update.
//#define PRED_COM_OPT1 
...
pthread_mutex_t mutex_counter_f = PTHREAD_MUTEX_INITIALIZER;


//number of neighbours for each molecule
int num_interactions [NUM_PARTICLES];
...
volatile int counter_f [NUM_PARTICLES];
volatile int counter_x [NUM_PARTICLES];
...
//for each molecule, this shows how many interactions have been computed so 
//far for that molecule; after ALL interactions are done for a given molecule,
//the value in this array for that molecule will be identical to the value
//in num_interactions for that molecule.
int num_performed_interactions [NUM_PARTICLES];
...
int BuildNeigh( void* arg, int id )
{
  ...
  for ( i=id; i<numMoles; i+=NTHREADS)
  {
    ...
    //WARNING!!!
    //these should be protected by mutexes, unless BuildNeigh is enclosed by barriers
    //or else threads in subsequent functions could start reading them before they
    //got initialized, and then the threads in here initialize them, while the other
    //threads in the subsequent functions were already processing these elements.
    #ifdef PRED_COM_OPT1
    counter_f [i] = 0;
    //initialize this to 1 so that ComputeForces can proceed in timesteps when BuildNeigh
    //is invoked (which includes the very first iteration); not sure if we should do 
    //this the subsequent times BuildNeigh is called, though.
    //I guess this means that BuildNeigh should _always_ be enclosed by barriers.
    counter_x [i] = 1;
    #endif
...
int ComputeForces( void* arg, int id )
{
  ...
  //the inter array is organized as follows: it contains pairs of molecules (i,j),
  //with i always being smaller than j; in other words, it will be like this:
  //(1,2), (1,3), ... (1, n-1), (2, 3), (2, 5), ... etc.
  ...
  //This is NOT ok; think what happens when some threads are stuck in 
  //a different parallel region; this array (num_performed_interactions)
  //doesn't get initialized for some molecules, and some threads that _are_ in 
  //this parallel region may process one such molecule, without 
  //num_performed_interactions for that molecule having been initialized; 
  //the cause for this problem is that you don't know which threads will process 
  //which molecules in the loop that goes through the interaction array.
  //the only way I can think of how to do this, is with a barrier...which defeats 
  //the purpose. Hmm, I think I have an idea: I reset this when I have finished
  //with each molecule.
  //for (i = 0; i < numMoles; i += NTHREADS) {
  //   num_performed_interactions [i] = 0;
  //}
  ...
  for(ii=id; ii<ninter; ii+=NTHREADS) {
    ...
    //WARNING: assumes BuildNeigh is surrounded by barriers!
    i = inter[INDX(ii,0)];
    j = inter[INDX(ii,1)];
    ...
    //Note that you must wait until the counter_x for both molecules are THREE, not ONE.
    //The reason for that is that the 3 coordinates for each molecule (x, y, z) are
    //updated separately in UpdateCoordinates. So you have to wait until all three
    //coordinates for a molecule have been updated, before you can start processing it.
    #ifdef PRED_COM_OPT1
    //while ((counter_x[i] != 3) && (counter_x[j] != 3));

    //printf("id, %d, counter_x_%03d, %d, counter_x_%03d, %d, counter_f_%03d, %d, counter_f_%03d, %d\n", id, i, counter_x[i], j, counter_x[j], i, counter_f[i], j, counter_f[j]);
    while ((counter_x[i] != 1) || (counter_x[j] != 1));
    #endif
    //printf ("id, %d, %03d, %03d, doneCompute\n", id, i, j); fflush (0);
    ...
    if ( rd < cutoffSquare ) {
      ...
      //we can make this mutex more fine-grained to reduce contention...
    ...
    #ifdef PRED_COM_OPT1
    //we have completed another interaction both for molecule i, and for molecule j.
    atomic_inc (&num_performed_interactions [i]);
    atomic_inc (&num_performed_interactions [j]);
    //**IFF** we are done with molecule i, we reset its counter_x for next 
    //call of ComputeForces; must be atomic write. Same for molecule j.
    //we also reset the num_performed_interactions for molecule i here (for
    //the next invocation of this method).
    
    //question: could it be possible that some of these following statements
    //are reordered by the cpu or compiler - hmmm, even if it was, I don't see
    //how that would cause threads to get stuck in Update waiting for a 
    //counter_f that never gets incremented to the correct value.
    
    if (num_performed_interactions [i] == num_interactions [i]) {
       set_mb (&num_performed_interactions [i], 0);
       set_mb (&counter_x[i], 0);
    }
    if (num_performed_interactions [j] == num_interactions [j]) {
      set_mb (&num_performed_interactions [j], 0);
      set_mb (&counter_x[j], 0);
    }
     pthread_mutex_lock (&mutex_counter_f);
    ++counter_f[i];
    ++counter_f[j];
    pthread_mutex_unlock (&mutex_counter_f);
    #endif
  ...
  //WARNING: these can stay here ONLY if PrintResults is enclosed by barriers;
  //in other words, as long as these are here, we cannot have threads going past
  //the timestep boundary (we cannot have some threads in current timestep and some
...
int Update( void* arg, int id )
{
  ...
  for ( i = id; i < numMoles; i += NTHREADS) 
  {
    ...
    //note that the read is not atomic, but the write to counter_f[i] _is_ atomic
    //(in ComputeForces).
    //Note that i goes from 0 to 3*numMoles, so you can't use i as the index
    //for num_interactions or counter_f or counter_x (thanks Mike for catching it).
    //I think i%numMoles should do it.
    #ifdef PRED_COM_OPT1
    //_tmp = num_interactions [i % numMoles];
    //while (counter_f[i % numMoles] != _tmp);
    _tmp = num_interactions [i];
    
    /*
    //debug
    if (counter_f [i] != _tmp) {
       printf ("id, %d, counter_f[%03d], %d, _tmp, %d, num_interc[%03d], %d\n", id, i, counter_f[i], _tmp, i, num_interactions[i]); fflush (0);
    }
    //debug end
    */
    while (counter_f[i] != _tmp) {
    ...
    //ComputeForces thread can start messing with data)
    #ifdef PRED_COM_OPT1
    //must be atomic; doesn't have to be at the end, only before incrementing counter_x
    //try some other way instead of set_mb; use mutex, etc
    pthread_mutex_lock (&mutex_counter_f);
    counter_f[i] = 0;
    //set_mb (&counter_f[i], 0);
    pthread_mutex_unlock (&mutex_counter_f);
...
void * do_thread_work (void * _id) {
  ...
  for ( tstep=0; tstep< NUMBER_TIMESTEPS; tstep++) 
  {
    ...
    if ( tstep % neighUpdate == 0) 
    {
      ...
      //I don't know how to do this other than here, between barriers; ideally,
      //it should be done without barriers somehow. 
      //(Somehow, and inside BuildNeigh, I guess?...)
      for (i = id; i < numMoles; i += NTHREADS) {
         num_interactions [i] = 0;
      }
      
      //this barrier needed because of the single-threaded code above
      //ALSO (especially) because we need to finish initializing num_interactions
      //before we can start the main loop in BuildNeigh.
      barrier_wait (&barrier);
      ...
      if (id ==0)     {
          for (i = 0; i < ninter ;i++ )
          {
               debug_inter[inter[INDX(i,0)]] ++;
               debug_inter[inter[INDX(i,1)]] ++;
               printf("adding %d/%d\n", inter[INDX(i,0)], inter[INDX(i,1)]);
          }
       for (i = 0; i < numMoles; i++) {
            printf ("debug_inter[%d]: %d\n", i, debug_inter[i]);
         }
    ...
    barrier_wait (&barrier);
    #endif
    //printf("\nstep %d: thread %d finished ComputeForces\n", tstep, id);
/*
    //debug
    #if 0
    barrier_wait (&barrier);
    if (id == 0) {
      for (i = 0; i < numMoles; i ++) {
         printf ("%d %d\n", num_interactions[i], counter_f[i]);
      }
    }
    #endif
    //debug end
      ...
      for (i = 0; i < numMoles; i ++) {
         printf ("id %d: num_interact[%d]: %d, counter_f[%d]: %d\n", id, i, num_interactions[i], i, counter_f[i]);
      }
      */
    ...
    Update(NULL, id);
    // printf("\nstep %d: thread %d finished Update\n", tstep, id);
...
int main( int argc, char *argv[])
{
  ...
  */
  ...
  //burceam: initialize num_performed_interactions for the very first time;
  //from now on, it will be reset in ComputeForces.
  for (i = 0; i < numMoles; i++) {
     num_performed_interactions [i] = 0;
  }
